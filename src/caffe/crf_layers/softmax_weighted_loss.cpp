...
#include "caffe/crf_layers/softmax_weighted_loss_layer.hpp"
...
namespace caffe {
    ...
    void SoftmaxWithWeightedLossLayer<Dtype>::Reshape(
        ...
        << "size of weight must match size of label ";
        if (top.size() >= 2) {
            // softmax output
    ...
    void SoftmaxWithWeightedLossLayer<Dtype>::Forward_cpu(
        ...
        for (int i = 0; i < outer_num_; ++i) {
            for (int j = 0; j < inner_num_; j++) {
                ...
                Dtype weight_value = weight[i * inner_num_ + j];
                ...
                loss -= weight_value * log(std::max(prob_data[i * dim + label_value * inner_num_ + j],
                                     Dtype(FLT_MIN)));
                ++count;
                sum_weight = sum_weight + weight_value;
            }
        }
