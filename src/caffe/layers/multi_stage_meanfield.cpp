...
void MultiStageMeanfieldLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->blobs_.size() > 0) {
    ...
    for (int i = 0; i < channels_; i++) {
        float a;
        fscanf(pFile, "%f", &a);
        this->blobs_[0]->mutable_cpu_data()[i * channels_ + i]=a;
    }
    fclose(pFile);
    ...
    for (int i = 0; i < channels_; i++) {
        float a;
        fscanf(pFile, "%f", &a);
        this->blobs_[1]->mutable_cpu_data()[i * channels_ + i] = a;
    }
    fclose(pFile);
...
void MultiStageMeanfieldLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if(!parameter_printed_)
  {
      parameter_printed_ = true;
      LOG(INFO)<<"alpha, beta, gamma = "<<theta_alpha_<<" "<<theta_beta_<<" "<<theta_gamma_;
      std::cout<<"w1"; //bilateral
      for (int i = 0; i < channels_; i++) {
          std::cout<<" "<<this->blobs_[1]->cpu_data()[i * channels_ + i];
      }
      std::cout<<std::endl<<"w2"; //spatial
      for (int i = 0; i < channels_; i++) {
          std::cout<<" "<<this->blobs_[0]->cpu_data()[i * channels_ + i];
      }
      std::cout<<std::endl;
  }
